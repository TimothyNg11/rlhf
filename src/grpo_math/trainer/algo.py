...
def truncated_is_weights(
    old_logprobs: torch.Tensor, sampling_logprobs: torch.Tensor, *, cap: float
) -> torch.Tensor:
    """Truncated importance-sampling weights ``min(exp(old_lp - sampling_lp), cap)``.

    ``old_logprobs`` are the training-engine (HF) recompute; ``sampling_logprobs``
    are the actual rollout/inference engine (vLLM) logprobs of the same tokens.
    The one-sided cap lets the ratio fall freely below 1 but bounds it above at
    ``cap`` (typical 1.5-5.0), trading a little bias for bounded variance
    (verl rollout-correction; Yao et al. 2025). Shape-preserving; no grad."""
    return torch.exp(old_logprobs - sampling_logprobs).clamp(max=cap)
...
def grpo_microbatch_loss(
    ...
    *,
    ...
    tis_weight: torch.Tensor | None = None,  # [B, T] (no grad) truncated IS weights, or None
) -> MicrobatchLossOut:
    ...
    ``tis_weight`` (truncated importance sampling): when the completions were
    generated by a different engine than the training forward (vLLM rollouts vs
    HF recompute), the recomputed ``old_logprobs`` are NOT the true behavior
    policy -- the sampler is -- so the PG estimate carries an uncorrected
    off-policy bias that compounds into entropy blow-up / collapse (Yao et al.
    2025, "Your Efficient RL Framework Secretly Brings You Off-Policy RL
    Training"; verl rollout-correction; TRL's default
    vllm_importance_sampling_correction). ``tis_weight[b,t] = min(exp(old_lp -
    sampling_lp), C)`` is a detached per-token coefficient on the PG term (the
    KL penalty is a separate regularizer and is NOT scaled), matching TRL's
    ordering. ``None`` disables it (weight 1 everywhere).

    ``per_token = tis_weight * pg + kl_coef * low_var_kl(logprobs, ref_logprobs)``.
    ...
    if tis_weight is not None:
        pg = tis_weight.detach() * pg
